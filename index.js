...
async function onMessageSwiped(data) {
    ...
    // TRACKER FIX: Remove old tracker display and snapshot for this message
    ...
    if (extensionSettings.trackerEnabled && extensionSettings.trackerInline) {
        ...
        // Wait for message content to stabilize before rebuilding tracker
        // (similar to working version's swipe wait logic)
        (async () => {
            ...
            while (Date.now() - start < maxWaitMs) {
                ...
                const mes = readMes();
                if (!mes || mes === initial) continue; // still same as before swipe
                if (mes === lastSeen) stableCount++; 
                else { stableCount = 1; lastSeen = mes; }
                if (stableCount >= 2) {
                    // Message content has stabilized - wait a bit more for DOM render
                    await new Promise(res => setTimeout(res, 150));
                    break;
                }
            }
            
            // Clear lock again in case another event set it, then rebuild
            window.FuckTrackerSnapshots.processing.delete(key);
...
async function onCharacterMessageRendered(eventArg) {
    ...
    if (!snapshot) {
        console.log(`[${MODULE_NAME}] [FUCKTRACKER] No snapshot found, attempting DOM fallback parse`);
        ...
        if (match) {
            let jsonStr = match[1];
            console.log(`[${MODULE_NAME}] [FUCKTRACKER] Found JSON in DOM (raw):`, jsonStr.substring(0, 200));
            ...
            jsonStr = jsonStr.replace(/&#39;/g, "'");
            jsonStr = jsonStr.replace(/&nbsp;/g, ' ');
            // Strip any remaining HTML tags that might have snuck in
            jsonStr = jsonStr.replace(/<[^>]*>/g, '');
            // Trim whitespace
            ...
            console.log(`[${MODULE_NAME}] [FUCKTRACKER] No JSON block found in DOM either`);
        }
    }
