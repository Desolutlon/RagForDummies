...
async function init() {
    ...
    if (typeof eventSource !== 'undefined') {
        console.log(`[${MODULE_NAME}] Using eventSource for events`);
        ...
        eventSource.on('CHARACTER_MESSAGE_RENDERED', onMessageReceived);
    }
    
    // Method 2: Try direct window event listeners
    if (typeof window !== 'undefined') {
        console.log(`[${MODULE_NAME}] Registering window event listeners`);
        window.addEventListener('message_sent', (e) => {
            console.log(`[${MODULE_NAME}] Window message_sent event:`, e);
