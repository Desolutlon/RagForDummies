...
function getQueryMessage(context, idxOverride = null) {
    ...
    // KEY FIX: If the last message is from a CHARACTER (not user), 
    // we're likely generating/swiping their response.
    // In this case, query based on what they're RESPONDING TO (the message before).
    if (!lastMsg.is_user && lastMsgIdx > 0) {
        ...
        for (let i = lastMsgIdx - 1; i >= 0; i--) {
            ...
            // Found the message the character is responding to
            console.log('[' + MODULE_NAME + '] Query: Using PREVIOUS message (idx ' + i + ', ' + (prevMsg.is_user ? 'user' : prevMsg.name || 'char') + ') instead of last char message');
            return prevMsg;
        }
    }
    
    // Otherwise return the last message (user just sent a message)
    return lastMsg;
}
