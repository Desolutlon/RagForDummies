...
async function injectContextBeforeGeneration(data) {
    ...
    // Get the ACTUAL chat from SillyTavern context (not data.chat which is API format)
    let context;
    if (typeof SillyTavern !== 'undefined' && SillyTavern.getContext) {
        context = SillyTavern.getContext();
    } else if (typeof getContext === 'function') {
    ...
    // Find the last message (user or character) - using SillyTavern format (.mes, .is_user)
    ...
    // Create a system message entry (matching the working extension's format)
    const ragEntry = {
        name: 'System',
        is_user: false,
        is_system: true,
        mes: retrievedContext,
        send_date: Date.now()
    };
    
    // Determine insert position
    let insertIndex;
    if (extensionSettings.injectionPosition === 'after_messages') {
        // Insert N messages from the end
        const depth = extensionSettings.injectAfterMessages || 3;
        insertIndex = Math.max(0, chat.length - depth);
    } else {
        // Default: insert near the end (like the working extension)
        insertIndex = Math.max(0, chat.length - 3);
    }
    
    // Insert into the actual chat array
    chat.splice(insertIndex, 0, ragEntry);
    
    console.log('[' + MODULE_NAME + '] Injected at index ' + insertIndex + ' (chat length now: ' + chat.length + ')');
