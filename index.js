...
async function injectContextBeforeGeneration(data) {
    ...
    // Tracker calls typically have:
    // 1. Very small chat array (< 3 messages)
    // 2. A specific identifier or quiet mode flag
    // 3. Or come from a non-main generation context
    if (data && data.chat && Array.isArray(data.chat)) {
        console.log('[' + MODULE_NAME + '] data.chat length:', data.chat.length);
        
        // Only skip if VERY small (< 3) or if there's a clear Tracker identifier
        if (data.chat.length < 3) {
            // Check if this looks like a real generation (has user messages)
            const hasUserMessages = data.chat.some(function(m) {
                return m && m.is_user === true;
            });
            
            if (!hasUserMessages) {
                console.log('[' + MODULE_NAME + '] Skipping - no user messages in small chat array (likely extension call)');
                return;
            }
        }
        
        // Skip if chat array is empty or clearly not a real generation
