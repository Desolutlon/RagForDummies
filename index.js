...
function extractProperNouns(text) {
    ...
    const commonWords = new Set([
        ...
        'just', 'also', 'now', 'here', 'there', 'always', 'never', 'sometimes', 'often', 'usually',
        // Common verbs
        ...
        'oh', 'ah', 'um', 'uh', 'hey', 'hi', 'hello', 'bye', 'yes', 'no', 'yeah', 'okay', 'ok', 'well', 'like',
        // Swear words (often capitalized for emphasis but not proper nouns)
        'fuck', 'shit', 'damn', 'hell', 'ass', 'crap', 'god', 'omg', 'wtf', 'lol', 'lmao',
        // Time words
        ...
        'one', 'two', 'three', 'something', 'nothing', 'everything', 'anything', 'someone', 'anyone', 'everyone', 'nobody'
    ]);
    ...
    // Also look for multi-word proper nouns (e.g., "New York", "San Francisco")
    // Match patterns like "Word Word" where both are capitalized
    const multiWordPattern = /(?:^|[.!?]\s+)(?:[A-Z][a-z]+\s+){1,2}[A-Z][a-z]+/g;
    const multiMatches = text.match(multiWordPattern);
    if (multiMatches) {
        multiMatches.forEach(function(match) {
            const cleaned = match.replace(/^[.!?\s]+/, '').trim();
            // Only add if it's not just sentence start
...
async function searchVectors(collectionName, vector, limit, scoreThreshold, properNouns) {
    ...
    try {
        ...
        if (properNouns.length > 0) {
            ...
            // Build filter: match ANY of the proper nouns (should = OR logic)
            const filter = {
            ...
            try {
                ...
                filteredResults = filteredResult.result || [];
                console.log('[' + MODULE_NAME + '] Run 1 returned ' + filteredResults.length + ' filtered results');
...
function extractPayload(message, messageIndex, chatIdHash) {
    ...
    // Include character name, any names in the message, locations, etc.
    const messageProperNouns = extractProperNouns(message.mes || '');
    
    // Also add character name and location as searchable terms
    const additionalNouns = [];
    if (message.name) {
        additionalNouns.push(message.name.toLowerCase());
    }
    if (tracker.Characters && tracker.Characters[message.name] && tracker.Characters[message.name].Location) {
        additionalNouns.push(tracker.Characters[message.name].Location.toLowerCase());
    }
    // Add characters present as searchable
    charactersPresent.forEach(function(char) {
        additionalNouns.push(char.toLowerCase());
    });
    
    // Combine and dedupe
    const allNouns = new Set(messageProperNouns.concat(additionalNouns));
    ...
    return {
        ...
        proper_nouns: Array.from(allNouns) // For hybrid search filtering
    };
}
...
async function injectContextBeforeGeneration(data) {
    console.log('[' + MODULE_NAME + '] 🚀 injectContextBeforeGeneration called with data:', data);
            ...
            try {
                ...
                if (properNouns.length > 0) {
                    ...
                    try {
                        ...
                        filteredResults = filteredResult.result || [];
                        log('Run 1 returned: ' + filteredResults.length + ' results');
                        ...
                        if (filteredResults.length > 0) {
                            filteredResults.forEach(function(r, idx) {
                                ...
                                log('  [' + idx + '] Score: ' + r.score.toFixed(3) + ' | Character: ' + r.payload.character_name + ' | Matched: ' + (matchedNouns.length > 0 ? matchedNouns.join(', ') : 'none'));
                            });
                        }
                    } catch (filterError) {
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // Try multiple event names since different ST versions use different names
        ...
        generationEvents.forEach(function(eventName) {
            eventSourceToUse.on(eventName, function(data) {
                console.log('[' + MODULE_NAME + '] 🎯 Generation event fired: ' + eventName);
                injectContextBeforeGeneration(data);
            });
        });
        
        console.log('[' + MODULE_NAME + '] Registered listeners for events: ' + generationEvents.join(', '));
