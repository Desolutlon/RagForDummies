...
async function onMessageReceived(messageData) {
    ...
    if (!extensionSettings.enabled || !extensionSettings.injectContext) {
        console.log('[' + MODULE_NAME + '] Injection disabled, skipping');
        return;
    }
    
    const chatId = getCurrentChatId();
    if (!chatId || !currentChatIndexed) {
        console.log('[' + MODULE_NAME + '] Not ready for injection - chatId:', chatId, 'indexed:', currentChatIndexed);
        return;
    }
    
    console.log('[' + MODULE_NAME + '] Triggering context injection...');
