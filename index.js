...
const INJECTION_DEBOUNCE_MS = 2000; // Minimum time between injections
...
async function retrieveContext(query, chatIdHash, isGroupChat) {
    ...
    try {
        ...
        // Extract proper nouns from query for hybrid search
        const queryProperNouns = extractProperNouns(query);
        console.log('[' + MODULE_NAME + '] Query proper nouns extracted: ' + 
            (queryProperNouns.length > 0 ? queryProperNouns.join(', ') : '(none found)'));
        
        const queryEmbedding = await generateEmbedding(query);
        
        // Use hybrid search with proper nouns
        const results = await searchVectors(
            collectionName,
            queryEmbedding,
            extensionSettings.retrievalCount,
            extensionSettings.similarityThreshold,
            queryProperNouns  // Pass proper nouns for filtering
        );
...
async function onMessageSent(messageData) {
    ...
    // Reset generation ID to allow new injection for the response
    lastInjectionGenerationId = null;
...
async function onMessageReceived(messageData) {
    ...
    // NOTE: Do NOT call injectContextBeforeGeneration here!
    // Generation events (GENERATE_BEFORE_COMBINE_PROMPTS etc.) handle injection
    // Calling it here would cause double/triple injection
    
    console.log('[' + MODULE_NAME + '] ===== MESSAGE RECEIVED HANDLER COMPLETE =====');
}

// Track last injection to prevent duplicates
let lastInjectionGenerationId = null;

async function injectContextBeforeGeneration(data) {
    // Check if this is likely a Tracker or other extension call (small chat array)
    // Main generation typically has many messages, Tracker sends minimal
    if (data && data.chat && Array.isArray(data.chat) && data.chat.length <= 5) {
        console.log('[' + MODULE_NAME + '] Skipping - likely Tracker/extension call (only ' + data.chat.length + ' messages in data.chat)');
        return;
    }
    
    // Also skip if dryRun is true
    if (data && data.dryRun === true) {
        console.log('[' + MODULE_NAME + '] Skipping - dryRun mode');
        return;
    }
    
    // Create a generation ID based on chat length and last message
    // This helps prevent duplicate injections for the same generation
    let generationId = 'unknown';
    if (data && data.chat && data.chat.length > 0) {
        const lastMsg = data.chat[data.chat.length - 1];
        const msgPreview = lastMsg && lastMsg.content ? lastMsg.content.substring(0, 50) : '';
        generationId = data.chat.length + '_' + msgPreview;
    }
    
    if (generationId === lastInjectionGenerationId) {
        console.log('[' + MODULE_NAME + '] Skipping - already injected for this generation');
        return;
    }
    
    console.log('[' + MODULE_NAME + '] 🚀 injectContextBeforeGeneration called (chat length: ' + (data && data.chat ? data.chat.length : 'N/A') + ')');
    ...
    try {
        ...
        // Look for messages with 'mes' property that are actual user/character messages
        ...
        // Get the last 2-3 actual messages for query (includes both user AND character)
        const recentMessages = actualMessages.slice(-3);
        
        // Build query from recent messages
        let query = recentMessages.map(function(m) { return m.mes; }).join(' ');
        
        console.log('[' + MODULE_NAME + '] Building query from ' + recentMessages.length + ' recent messages');
        recentMessages.forEach(function(m, idx) {
            console.log('[' + MODULE_NAME + ']   Message ' + idx + ' (' + (m.is_user ? 'USER' : 'CHAR') + '): "' + m.mes.substring(0, 50) + '..."');
        });
        
        // Truncate query if too long (embeddings work better with focused text)
        if (query.length > 1500) {
            query = query.substring(0, 1500);
            console.log('[' + MODULE_NAME + '] Query truncated to 1500 chars');
        }
        ...
        if (retrievedContext) {
            ...
            // THE CORRECT WAY - setExtensionPrompt is on the context object!
            let injected = false;
            ...
            if (context.setExtensionPrompt && typeof context.setExtensionPrompt === 'function') {
                try {
                    ...
                    injected = true;
                    console.log('[' + MODULE_NAME + '] ✓✓✓ context.setExtensionPrompt() called successfully!');
                    updateUI('status', 'Context injected! (' + retrievedContext.length + ' chars)');
                } catch (e) {
                    console.error('[' + MODULE_NAME + '] context.setExtensionPrompt() threw error:', e);
                }
            } else {
                console.error('[' + MODULE_NAME + '] ✗✗✗ context.setExtensionPrompt not found!');
                console.error('[' + MODULE_NAME + '] Context object:', context);
            }
            ...
            if (injected) {
                ...
                lastInjectionGenerationId = generationId;
                
                // Verify it's in the array
                if (context.extensionPrompts) {
                    console.log('[' + MODULE_NAME + '] Current extensionPrompts array:', context.extensionPrompts);
                }
            } else {
                updateUI('status', 'Context retrieved but injection failed');
                console.log('[' + MODULE_NAME + '] ===== INJECTION FAILED =====');
            }
        } else {
            ...
            try {
                ...
                if (properNouns.length > 0) {
                    ...
                    try {
                        ...
                        if (falsePositives > 0) {
                            log('⚠️ Filtered out ' + falsePositives + ' false positives (Qdrant filter issue)');
                        }
                        ...
                        if (filteredResults.length > 0) {
                            filteredResults.forEach(function(r, idx) {
                                ...
                                log('  [' + idx + '] Score: ' + r.score.toFixed(3) + ' | Character: ' + r.payload.character_name + ' | Matched: ' + matchedNouns.join(', '));
                            });
                        }
                    } catch (filterError) {
                    ...
                    log('RUN 1: Skipped (no proper nouns to filter by)');
                }
                log('');
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // Use ONLY chat_completion_prompt_ready as it's the most reliable
        // The function itself will filter out Tracker/extension calls
        eventSourceToUse.on('chat_completion_prompt_ready', function(data) {
            console.log('[' + MODULE_NAME + '] 🎯 Generation event fired: chat_completion_prompt_ready (chat length: ' + (data && data.chat ? data.chat.length : 'N/A') + ')');
