...
async function pollForNewMessages() {
    ...
    if (isIndexing) {
        console.log('[' + MODULE_NAME + '] Already indexing, skipping poll');
        return;
    }
    ...
    try {
        ...
        if (lastChatId !== chatId) {
            console.log('[' + MODULE_NAME + '] Chat changed from ' + lastChatId + ' to ' + chatId + ', resetting');
            lastChatId = chatId;
            currentChatIndexed = false;
            lastMessageCount = 0;
            indexedMessageIds.clear();
            ...
            if (existingPoints > 0) {
                console.log('[' + MODULE_NAME + '] Chat already has ' + existingPoints + ' indexed points in Qdrant');
                currentChatIndexed = true;
                lastMessageCount = context.chat.length;
        ...
        if (currentMessageCount > lastMessageCount) {
            console.log('[' + MODULE_NAME + '] New message(s) detected: ' + lastMessageCount + ' -> ' + currentMessageCount);
