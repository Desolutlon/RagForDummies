    ...
    try {
        ...
        if (retrievedContext) {
            ...
            if (typeof setExtensionPrompt === 'function') {
                ...
                console.error('[' + MODULE_NAME + '] setExtensionPrompt not available!');
...
function attachEventListeners() {
    ...
    if (testRetrievalBtn) {
        testRetrievalBtn.addEventListener('click', async function() {
            console.log('[' + MODULE_NAME + '] TEST RETRIEVAL CLICKED!');
            updateUI('status', 'Testing context retrieval...');
            try {
                const chatId = getCurrentChatId();
                console.log('[' + MODULE_NAME + '] Test - Chat ID:', chatId);
                
                if (!chatId) {
                    updateUI('status', '✗ No active chat found');
                    console.log('[' + MODULE_NAME + '] Test failed: No chat ID');
                    return;
                }
                
                console.log('[' + MODULE_NAME + '] Test - Chat indexed:', currentChatIndexed);
                
                if (!currentChatIndexed) {
                    updateUI('status', '✗ Chat not indexed yet. Click "Index Current Chat" first.');
                    console.log('[' + MODULE_NAME + '] Test failed: Chat not indexed');
                    return;
                }
                
                const isGroupChat = isCurrentChatGroupChat();
                console.log('[' + MODULE_NAME + '] Test - Is group chat:', isGroupChat);
                
                const context = SillyTavern.getContext();
                const recentMessages = context.chat.slice(-3);
                const query = recentMessages.map(function(m) { return m.mes; }).join(' ');
                
                console.log('[' + MODULE_NAME + '] Test - Query length:', query.length);
                console.log('[' + MODULE_NAME + '] Test - Query preview:', query.substring(0, 100));
                
                const retrievedContext = await retrieveContext(query, chatId, isGroupChat);
                
                console.log('[' + MODULE_NAME + '] Test - Retrieved context length:', retrievedContext ? retrievedContext.length : 0);
                
                if (retrievedContext) {
                    updateUI('status', '✓ Retrieved ' + retrievedContext.length + ' chars of context');
                    console.log('[' + MODULE_NAME + '] ===== RETRIEVED CONTEXT =====');
                    console.log(retrievedContext);
                    console.log('[' + MODULE_NAME + '] ===== END CONTEXT =====');
                } else {
                    updateUI('status', '✗ No context found (try lowering similarity threshold to 0.5)');
                    console.log('[' + MODULE_NAME + '] No context retrieved - check similarity threshold');
                }
            } catch (error) {
                updateUI('status', '✗ Retrieval failed: ' + error.message);
                console.error('[' + MODULE_NAME + '] Retrieval test FAILED:', error);
    ...
    if (stopIndexingBtn) {
        stopIndexingBtn.addEventListener('click', function() {
            ...
            updateUI('status', 'Stopping indexing...');
        });
    }
