...
async function onMessageSent(messageData) {
    ...
    // PRE-COMPUTE RAG context and SET IT NOW (before generation starts)
    if (extensionSettings.injectContext && currentChatIndexed) {
        console.log('[' + MODULE_NAME + '] Pre-computing RAG context for upcoming generation...');
        try {
            const context = SillyTavern.getContext();
            ...
            if (lastMessage) {
                ...
                console.log('[' + MODULE_NAME + '] Query: "' + query.substring(0, 80) + '..."');
                ...
                if (retrievedContext) {
                    ...
                    // SET EXTENSION PROMPT NOW - before generation starts
                    // This must happen BEFORE the prompt is assembled
                    if (typeof extension_prompts !== 'undefined') {
                        ...
                        console.log('[' + MODULE_NAME + '] Set extension_prompts[' + MODULE_NAME + '] with ' + retrievedContext.length + ' chars');
                        console.log('[' + MODULE_NAME + '] Position: ' + position + ', Depth: ' + depth);
                        updateUI('status', 'RAG context ready (' + retrievedContext.length + ' chars)');
                    } else {
                        console.log('[' + MODULE_NAME + '] WARNING: extension_prompts not available');
                    }
                } else {
                    console.log('[' + MODULE_NAME + '] No relevant context found');
                    // Clear any old prompt
                    if (typeof extension_prompts !== 'undefined' && extension_prompts[MODULE_NAME]) {
                        delete extension_prompts[MODULE_NAME];
                    }
                }
            }
        } catch (error) {
            console.error('[' + MODULE_NAME + '] Pre-computation failed:', error);
        }
    }
...
async function injectContextBeforeGeneration(data) {
    // This event fires AFTER prompt assembly, so we can only verify here
    // Actual injection happens in onMessageSent BEFORE generation
    
    // Skip Tracker and other extension calls
    if (data && data.chat && Array.isArray(data.chat) && data.chat.length <= 5) {
        return; // Silent skip for Tracker
    }
    ...
    // Just log verification that our prompt is in the extension_prompts
    if (typeof extension_prompts !== 'undefined' && extension_prompts[MODULE_NAME]) {
        const prompt = extension_prompts[MODULE_NAME];
        console.log('[' + MODULE_NAME + '] Generation event - RAG context is set (' + (prompt.value ? prompt.value.length : 0) + ' chars)');
    } else {
        console.log('[' + MODULE_NAME + '] Generation event - No RAG context set');
    }
}
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // chat_completion_prompt_ready fires for all generations - we filter Tracker by chat array size
