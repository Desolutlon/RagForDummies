...
function attachEventListeners() {
    ...
    if (testInjectionBtn) {
        testInjectionBtn.addEventListener('click', async function() {
            ...
            console.log('[' + MODULE_NAME + '] Checking if setExtensionPrompt exists...');
            ...
            if (typeof setExtensionPrompt === 'function' || typeof window.setExtensionPrompt === 'function') {
                console.log('[' + MODULE_NAME + '] ✓ setExtensionPrompt is available');
                console.log('[' + MODULE_NAME + '] Manually triggering injection...');
                await injectContextBeforeGeneration();
                console.log('[' + MODULE_NAME + '] Check logs above for injection details');
            } else {
                console.error('[' + MODULE_NAME + '] ✗ setExtensionPrompt NOT AVAILABLE');
                console.error('[' + MODULE_NAME + '] This means SillyTavern extension API is not loaded or has changed');
                console.log('[' + MODULE_NAME + '] Searching for alternative globals...');
                
                const allGlobals = Object.keys(window);
                const promptRelated = allGlobals.filter(function(k) {
                    return k.toLowerCase().indexOf('prompt') !== -1 ||
                           k.toLowerCase().indexOf('extension') !== -1;
                });
                
                console.log('[' + MODULE_NAME + '] Prompt-related globals found:', promptRelated);
                updateUI('status', '✗ setExtensionPrompt not available - check console');
            }
