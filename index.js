...
/**
 * Get all participant names (user + characters) from context or chat data
 * These names should be excluded from proper noun extraction since they appear in almost every message
 * @param {Object} contextOrChat - SillyTavern context object or chat array
 * @returns {Set<string>} - Set of lowercase participant names
 */
function getParticipantNames(contextOrChat) {
    ...
    if (Array.isArray(contextOrChat)) {
        // It's a chat array, try to get context
    ...
    if (context) {
        ...
        // Also extract from chat messages themselves (catches all character names)
    ...
    // If we got a chat array directly, extract names from it
    if (Array.isArray(contextOrChat)) {
    ...
    try {
        ...
        if (rawFiltered.length > 0) {
            filteredResults = rawFiltered.filter(function(r) {
                ...
                return resultNouns.some(function(noun) {
...
function extractPayload(message, messageIndex, chatIdHash, participantNames) {
    if (participantNames === undefined) participantNames = new Set();
    ...
    const summary = (message.extra && message.extra.qvink_memory && message.extra.qvink_memory.memory) 
        ...
        : '';
    const summaryProperNouns = summary ? extractProperNouns(summary, participantNames) : [];
...
function getQueryMessage(context, idxOverride, generationType) {
    ...
    // If a specific index is provided, try that first
    ...
    // KEY FIX: Only skip to previous message during SWIPE or REGENERATE
    // During initial generation ('normal'), the new message doesn't exist yet,
    // so the last message IS what the AI should respond to.
    // During swipe, the message being replaced EXISTS in the chat,
    // so we need to skip it and query the previous message.
    ...
    if (isSwipeOrRegen && !lastMsg.is_user && lastMsgIdx > 0) {
        // Find the previous non-system message (what the character is responding to)
        for (let i = lastMsgIdx - 1; i >= 0; i--) {
            const prevMsg = context.chat[i];
            if (!prevMsg || !prevMsg.mes || !prevMsg.mes.trim()) continue;
            if (prevMsg.is_system) continue;
            console.log('[' + MODULE_NAME + '] Query: ' + generationType + ' detected - using PREVIOUS message (idx ' + i + ') instead of message being replaced');
            return prevMsg;
        }
    }
    
    // Normal generation: use the last message (what the AI will respond to)
...
async function indexChat(jsonlContent, chatIdHash, isGroupChat) {
    ...
    try {
        ...
        // Build set of participant names to exclude from proper noun extraction
        // This prevents user/character names from cluttering the hybrid search filter
        const participantNames = new Set();
        messages.forEach(function(msg) {
            if (msg.name && typeof msg.name === 'string') {
                participantNames.add(msg.name.toLowerCase());
                // Also add individual parts of multi-word names
                msg.name.toLowerCase().split(/\s+/).forEach(function(part) {
                    if (part.length >= 2) participantNames.add(part);
                });
            }
        });
        // Also try to get from current context (for user name)
        const contextNames = getParticipantNames(null);
        contextNames.forEach(function(name) { participantNames.add(name); });
        console.log('[' + MODULE_NAME + '] Excluding participant names from proper nouns: ' + Array.from(participantNames).join(', '));
...
async function indexSingleMessage(message, chatIdHash, messageIndex, isGroupChat) {
    ...
    try {
        ...
        // Get participant names to exclude from proper noun extraction
...
function attachEventListeners() {
    ...
    if (testInjectionBtn) {
        testInjectionBtn.addEventListener('click', async function() {
            ...
            await injectContextWithSetExtensionPrompt('normal');
    ...
    if (testHybridBtn) {
        testHybridBtn.addEventListener('click', async function() {
            ...
            try {
                ...
                // Get participant names to exclude
                const participantNames = getParticipantNames(null);
                log('Excluding participant names: ' + Array.from(participantNames).join(', '));
                
                const filterTerms = extractQueryFilterTerms(query, participantNames);
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        if (typeof injectContextWithSetExtensionPrompt === 'function') {
            eventSourceToUse.on('GENERATION_AFTER_COMMANDS', async function(type, opts, dryRun) {
                console.log('[' + MODULE_NAME + '] Event: GENERATION_AFTER_COMMANDS (type: ' + type + ')');
                await injectContextWithSetExtensionPrompt(type);
            });
