...
function extractProperNouns(text) {
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        for (let j = 1; j < words.length; j++) {
            ...
            // Check if it starts with capital and is at least 2 chars
            // Also filter out common words that are often capitalized mid-sentence
            if (cleaned.length >= 2 && /^[A-Z]/.test(cleaned)) {
                // Exclude common false positives (I, etc.)
                const upper = cleaned.toUpperCase();
                if (upper !== 'I' && upper !== 'OK' && upper !== 'AM' && upper !== 'PM') {
    ...
    // Also extract words after colons, quotes, and dashes that start with capitals
    const afterPunctuation = text.match(/[:\-"']\s*([A-Z][a-zA-Z]+)/g);
    if (afterPunctuation) {
        afterPunctuation.forEach(function(match) {
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // Use only ONE event to avoid multiple injections
        eventSourceToUse.on('GENERATE_BEFORE_COMBINE_PROMPTS', injectContextBeforeGeneration);
        // REMOVED: These cause duplicate injections
        // eventSourceToUse.on('IMPERSONATE_READY', injectContextBeforeGeneration);
        // eventSourceToUse.on('CHAT_COMPLETION_READY', injectContextBeforeGeneration);
