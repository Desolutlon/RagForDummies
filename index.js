...
async function injectContextBeforeGeneration(data) {
    ...
    try {
        ...
        console.log('[' + MODULE_NAME + '] Retrieving context for recent messages');
        ...
        if (retrievedContext) {
            ...
            console.log('[' + MODULE_NAME + '] Context preview:', retrievedContext.substring(0, 300));
            
            // Check all available prompt injection methods
            console.log('[' + MODULE_NAME + '] Checking available injection methods...');
            console.log('[' + MODULE_NAME + '] - setExtensionPrompt:', typeof setExtensionPrompt);
            console.log('[' + MODULE_NAME + '] - window.setExtensionPrompt:', typeof window.setExtensionPrompt);
            ...
            console.log('[' + MODULE_NAME + '] Relevant globals:', relevantGlobals);
            ...
            if (typeof setExtensionPrompt === 'function') {
                ...
                console.error('[' + MODULE_NAME + '] ✗✗✗ No injection method available!');
                console.error('[' + MODULE_NAME + '] This means SillyTavern API has changed or is not loaded');
                updateUI('status', 'Context retrieved but injection failed - check console');
            }
        } else {
            console.log('[' + MODULE_NAME + '] No relevant context found (similarity threshold too high?)');
            updateUI('status', 'No relevant context found');
        }
    } catch (error) {
        console.error('[' + MODULE_NAME + '] Context retrieval/injection failed:', error);
        updateUI('status', 'Context injection error - check console');
    }
}
...
async function pollForNewMessages() {
    ...
    if (isIndexing) {
        // Don't log during indexing, just skip
        return;
    }
    ...
    try {
        ...
        // If chat changed, reset everything
        if (lastChatId !== chatId) {
            ...
            // Check if this chat is already indexed in Qdrant
            const isGroupChat = isCurrentChatGroupChat();
            const prefix = isGroupChat ? 'st_groupchat_' : 'st_chat_';
            ...
            if (existingPoints > 0) {
                console.log('[' + MODULE_NAME + '] Chat has ' + existingPoints + ' points in Qdrant - marking as indexed');
                currentChatIndexed = true;
                lastMessageCount = context.chat.length;
        ...
        if (currentMessageCount > lastMessageCount) {
            console.log('[' + MODULE_NAME + '] New message detected: count ' + lastMessageCount + ' -> ' + currentMessageCount);
            ...
            // Inject context after new message
...
async function startPolling() {
    ...
    pollingInterval = setInterval(pollForNewMessages, 5000); // Reduced from 2 to 5 seconds
...
function showStopButton() {
    ...
    if (btn) {
        ...
        console.error('[' + MODULE_NAME + '] Stop button element not found!');
    }
}
...
function hideStopButton() {
    ...
    if (btn) {
        ...
        console.log('[' + MODULE_NAME + '] Stop button hidden');
    }
}
    ...
    const html = `
        <div id="ragfordummies_container" class="inline-drawer">
            ...
            <div class="ragfordummies-section">
                ...
                <button id="ragfordummies_stop_indexing" class="menu_button ragfordummies-stop-btn" style="display:none; background: #d9534f;">Stop Indexing</button>
...
async function init() {
    ...
    // Debug: Check if elements exist and setup dropdown
    setTimeout(function() {
        ...
        toggle.off('click').on('click', function(e) {
            ...
            console.log('[' + MODULE_NAME + '] Toggled! Icon classes:', icon.attr('class'));
        });
    ...
    if (typeof eventSource !== 'undefined') {
        ...
        // Try various event names for generation/prompts
        const eventNames = [
            'chat_loaded',
            'message_sent', 
            'message_received',
        ...
        eventNames.forEach(function(eventName) {
            eventSource.on(eventName, function(data) {
                ...
                } else if (eventName === 'GENERATE_BEFORE_COMBINE_PROMPTS' || eventName === 'EXTENSION_PROMPT_REQUESTED') {
