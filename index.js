...
async function injectContextBeforeGeneration(data) {
    // For early events, data might not have chat array - that's fine
    // We'll get chat from SillyTavern context instead
    
    // Skip if dryRun is true
    if (data && data.dryRun === true) {
        console.log('[' + MODULE_NAME + '] Skipping - dryRun mode');
        return;
    }
    
    console.log('[' + MODULE_NAME + '] 🚀 injectContextBeforeGeneration called');
...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // IMPORTANT: Must use GENERATE_BEFORE_COMBINE_PROMPTS (fires BEFORE prompt assembly)
        ...
        earlyEvents.forEach(function(eventName) {
        ...
        console.log('[' + MODULE_NAME + '] Registered listeners for: ' + earlyEvents.join(', '));
