...
/**
 * Extracts high-signal NOUNS from text using a dynamic limit for maximum relevance.
 * This is the new NLP-based keyword extractor.
 * @param {string} text The message text.
 * @param {Set<string>} excludeNames A set of names to ignore.
 * @returns {string[]} An array of the most relevant, high-precision noun keywords.
 */
...
function extractProperNouns(text, excludeNames) {
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        for (let j = 1; j < words.length; j++) {
            ...
            if (word.indexOf("'") !== -1 && !word.endsWith("'s")) continue;
            if (/^\d/.test(word)) continue;
            if (/\d/.test(word) && /[a-zA-Z]/.test(word)) continue;
            
            const cleaned = word.replace(/^[^a-zA-Z]+|[^a-zA-Z]+$/g, '').replace(/'s$/, '');
...
function attachEventListeners() {
    const settingIds = [
        ...
        'inject_context', 'injection_position', 'inject_after_messages', 'exclude_last_messages'
    ];
