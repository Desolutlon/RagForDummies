...
async function onMessageSent(messageData) {
    ...
    // Note: RAG context retrieval now happens during chat_completion_prompt_ready event
    // based on the user's actual message for relevance
...
async function injectContextBeforeGeneration(data) {
    ...
    // Get the LAST MESSAGE from data.chat for the query
    // Could be user or assistant (characters can reply to each other in group chats)
    let query = '';
    if (data && data.chat && Array.isArray(data.chat) && data.chat.length > 0) {
        const lastMessage = data.chat[data.chat.length - 1];
        query = lastMessage.content || '';
    }
    
    if (!query || query.length === 0) {
        console.log('[' + MODULE_NAME + '] No user message found for query');
        return;
    }
    ...
    console.log('[' + MODULE_NAME + '] Query from user message: "' + query.substring(0, 80) + '..."');
    
    // Retrieve context based on user's actual message
    ...
    // Inject directly into data.chat array
    const ragMessage = {
        role: 'system',
        content: retrievedContext
    };
    
    // Find where to inject based on settings
    let insertIndex = 0;
    
    if (extensionSettings.injectionPosition === 'before_main' || extensionSettings.injectionPosition === 'after_main') {
        // Insert after system messages but before user/assistant messages
        for (let i = 0; i < data.chat.length; i++) {
            if (data.chat[i].role === 'system') {
                insertIndex = i + 1;
            } else {
                break;
            }
        ...
        // Insert N messages from the end
    ...
    console.log('[' + MODULE_NAME + '] Injected at index ' + insertIndex);
