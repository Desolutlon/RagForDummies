...
async function injectContextBeforeGeneration(data) {
    ...
    const chatId = getCurrentChatId();
    if (!chatId || !currentChatIndexed) {
        console.log('[' + MODULE_NAME + '] Skipping context injection - not ready');
        return;
    }
    ...
    try {
        ...
        console.log('[' + MODULE_NAME + '] Retrieving context for query');
        
        // Retrieve context
        const retrievedContext = await retrieveContext(query, chatId, isGroupChat);
        
        if (retrievedContext) {
            console.log('[' + MODULE_NAME + '] Retrieved context:', retrievedContext.substring(0, 200) + '...');
            console.log('[' + MODULE_NAME + '] Attempting injection via setExtensionPrompt');
            ...
            if (typeof setExtensionPrompt === 'function') {
                ...
                console.warn('[' + MODULE_NAME + '] ✗ setExtensionPrompt not available');
                console.log('[' + MODULE_NAME + '] Available globals:', Object.keys(window).filter(function(k) { return k.indexOf('prompt') !== -1 || k.indexOf('extension') !== -1; }));
            }
        } else {
            console.log('[' + MODULE_NAME + '] No relevant context found');
        }
