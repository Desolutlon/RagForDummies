...
async function generateKoboldEmbedding(texts, isArray) {
    ...
    for (const text of texts) {
        const response = await fetch(extensionSettings.koboldUrl + '/api/extra/generate_embedding', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: text })
        });
        if (!response.ok) throw new Error('Kobold embedding failed: ' + response.status);
...
async function onMessageSwiped(data) {
    ...
    if (extensionSettings.trackerEnabled) {
        ...
        // Remove old tracker DOM element
        ...
        // Rebuild tracker after a short delay to let DOM update
        setTimeout(() => {
