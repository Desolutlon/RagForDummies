...
async function init() {
    ...
    if (eventSourceToUse) {
        ...
        // chat_completion_prompt_ready fires after prompt assembly - we filter Tracker by chat array size
        eventSourceToUse.on('chat_completion_prompt_ready', function(data) {
            console.log('[' + MODULE_NAME + '] Event fired: chat_completion_prompt_ready (chat length: ' + (data && data.chat ? data.chat.length : 'N/A') + ')');
