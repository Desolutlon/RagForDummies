...
async function injectContextBeforeGeneration(data) {
    ...
    try {
        ...
        // Get the last 2-3 actual messages for query context
        // Prioritize the LAST USER MESSAGE for the query
        const lastUserMessage = actualMessages.slice().reverse().find(function(m) {
            return m.is_user === true;
        });
        
        // Get last 2 messages for additional context (but user message is primary)
        const recentMessages = actualMessages.slice(-2);
        
        // Build query: prioritize user message, add recent context
        let query = '';
        if (lastUserMessage) {
            query = lastUserMessage.mes;
            console.log('[' + MODULE_NAME + '] Using last user message for query');
        } else if (recentMessages.length > 0) {
            query = recentMessages.map(function(m) { return m.mes; }).join(' ');
            console.log('[' + MODULE_NAME + '] Using last ' + recentMessages.length + ' messages for query');
        }
        ...
        console.log('[' + MODULE_NAME + '] Query text (' + query.length + ' chars): "' + query.substring(0, 200) + (query.length > 200 ? '...' : '') + '"');
    ...
    if (testRetrievalBtn) {
        testRetrievalBtn.addEventListener('click', async function() {
            ...
            try {
                ...
                // Get the last user message for primary query
                const lastUserMessage = actualMessages.slice().reverse().find(function(m) {
                    return m.is_user === true;
                });
                ...
                console.log('[' + MODULE_NAME + '] Query: "' + query.substring(0, 200) + (query.length > 200 ? '...' : '') + '"');
                console.log('[' + MODULE_NAME + '] ---');
